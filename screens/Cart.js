...
const Cart = () => {
  ...
  async function add(item) {
    ...
    arr[indexed].quantity++;
    // -----------price-----------

    if (arr[indexed].quantity == 1) {
      arr[indexed].price = arr[indexed].price * 2;
    } else {
      var diff = arr[indexed].price / (q - 1);
      arr[indexed].price = arr[indexed].price;
    }
          ...
          <View style={styles.card}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <TouchableOpacity onPress={() => min(item)}>
                <AntDesign name="minuscircle" size={20} color="#ff7700" />
              </TouchableOpacity>
            </View>
          </View>
        </View>
